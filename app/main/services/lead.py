...
from main.models.course import (
    Course as DBCourse,
)  # Renamed as DBCourse for consistency's sake
...
class LeadService(BaseService):
    ...
    def get_lead(self, db: Session, id: int) -> Lead:
        ...
        for attempt in db_lead.attempts:
            attempts.append(
                CourseAttempt(attempt.course.name, attempt_year=attempt.attempt_year)
            )
    ...
    def create_lead(self, db: Session, lead: LeadBase) -> Lead:
        ...
        for attempt in attempts:
            ...
            db_attempt = DBAttempt(
                ...
                attempt_year=attempt.attempt_year,
            )
            db.add(db_attempt)
        db.commit()
        db.refresh(db_lead)
